...
function svg() {
    return src('app/images/svg/*')
        .pipe(gulpif(isSprite, svgmin({
            js2svg: {
                pretty: true
            }
        }))) 
        .pipe(gulpif(isSprite, cheerio({
            run: function($) {
        ...
        .pipe(gulpif(isSprite, svgSprite({
            mode: {
                symbol: {
                    sprite: 'all.svg'
                }
...
exports.build         = series(cleanDist, scripts, concatScripts, parallel(html, styles, stylesVendor, images, svg, fonts))
